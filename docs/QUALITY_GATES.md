...
- `PYTHONPATH=src python3 scripts/acquire_vector_sources.py --output-dir <persistent external directory> --cache-dir <persistent cache directory>` for bounded ArcGIS feature acquisition, CRS/geometry checks, transfer-limit handling, and storage-safety checks;
- `PYTHONPATH=src python3 scripts/validate_vector_artifacts.py --manifest <manifest> --output <temporary report>` for external vector-manifest, artifact-count, CRS, geometry, and page-completeness validation;
- `PYTHONPATH=src python3 scripts/acquire_copernicus_dem.py --output-dir <persistent external directory>` for approved public GLO-30 tile acquisition and GeoTIFF/XML validation;
- `PYTHONPATH=src python3 scripts/cleanup_acquisition_storage.py --cache-dir <persistent cache> --output-root <persistent output>` to review abandoned cache/staging directories before any cleanup;
