...
| Terrain / slope | [ELVIS access](https://elevation.fsdf.org.au/), [NSW Elevation and Depth guidance](https://www.nsw.gov.au/environment-land-and-water/spatial-data-and-mapping/foundation-spatial-data-framework/elevation-and-depth), [Copernicus DEM GLO-30](https://dataspace.copernicus.eu/explore-data/data-collections/copernicus-contributing-missions/collections-description/COP-DEM), and [AWS Open Data Registry distribution](https://registry.opendata.aws/copernicus-dem/) | ELVIS is the selected discovery/order path for Australian elevation products. The live NSW multi-CRS FeatureServer was also checked and exposes only SpotHeight, RelativeHeight, and Contour feature layers, not a raster DEM. The approved GLO-30 fallback is publicly distributed as credential-free Cloud Optimized GeoTIFF tiles in the Copernicus AWS bucket; the implementation validates the tile headers and official XML sidecars. | **Selected with explicit fallback.** `config/model.json` requires ELVIS/NSW first and Copernicus second. The reproducible fallback acquisition writes four S1-intersecting tiles to external storage and publishes a manifest after validating EPSG:4326, one-arcsecond structure, nominal 30 m resolution, nodata `-32767`, and complete envelope coverage. |
...
| Native vegetation | [NSW State Vegetation Type Map (SVTM)](https://data.nsw.gov.au/data/dataset/nsw-state-vegetation-type-map), [ArcGIS feature service](https://mapprod3.environment.nsw.gov.au/arcgis/rest/services/VIS/SVTM_NSW_Extant_PCT/MapServer/3), [WMS display resource](https://mapprod3.environment.nsw.gov.au/arcgis/services/VIS/SVTM_NSW_Extant_PCT/MapServer/WMSServer), and [official Data.NSW/SEED resource](https://data.nsw.gov.au/data/dataset/nsw-state-vegetation-type-map/resource/38cb096e-25e4-42e1-9996-807765ec7c47) | Current release is C2.0.M2.2 (December 2025), licensed Creative Commons Attribution. The queryable `Plant Community Type with labels` layer exposes PCT and vegetation class/form attributes in EPSG:3308. A live S1 count returned 216,808 polygons. The official resource is configured as a guarded bulk-package alternative with 4 GB archive, 8 GB extracted-content, and 12 GB archive-plus-selected-extraction limits; Data.NSW reports no package size, its page describes the download package as MXD/layer-file symbology, and the direct resource currently returns an HTTP 202 web challenge. No complete analytical package artifact is verified. | **Recommended as a weighted environmental penalty.** The approved dataset and methodology are unchanged. The bulk delivery may be used only after vector geometry, required attributes, S1 coverage, and REST-count reconciliation are independently verified; mapping uncertainty and the distinction between mapped vegetation and legal clearing controls remain explicit. |
...
deferred from the static MVP. Terrain source selection is approved and the persistent
Copernicus fallback is now validated; raster processing remains Priority 3 work.
